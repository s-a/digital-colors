...
describe('highlightUrl', () => {
	...
	it('should handle URL with query parameters without values', () => {
		const inputText = 'Filter /api/filter?active&pending='
		const expectedPath = chalk.cyan('/api/filter')
		const expectedQuery = (
			chalk.cyan('?') +
      chalk.yellow('active') +
      chalk.cyan('&') +
      ...
      chalk.cyan('=')
		...
		const finalExpectedOutput = `Filter ${expectedPath}${expectedQuery}`
